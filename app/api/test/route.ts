...
export async function POST (req: Request) {
    try {
        ...
        for (const field of formFields) {
            if (!validQuestionTypes.includes(field.type)) {
                return NextResponse.json(
                    { error: `Invalid question type: ${field.type}` },
                    { status: 400 }
                )
            }
        }
        ...
        const questionOptionsToInsert = formFields.flatMap(
        ...
        if (questionOptionsToInsert.length > 0) {
            const { error: optionsError } = await supabase
                ...
                .insert(questionOptionsToInsert)
        ...
        for (const field of formFields) {
            ...
            if (questionId) {
                ...
                if (questionInsertError != null) {
                    ...
                    return NextResponse.json(
                        { error: questionInsertError.message },
                        { status: 500 }
                    )
                }
